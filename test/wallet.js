...
contract('Wallet', (accounts) => {
  ...
  it('test if failed transaction reverts contract values', async () => {
  ...
  it('getSenderApprovedTransactions should returns all transactions approvals to false but the approved one by the account', async () => {
    await wallet.createTransfer(20, accounts[3], {from: accounts[0]});
    await wallet.createTransfer(40, accounts[4], {from: accounts[0]});
    await wallet.createTransfer(60, accounts[5], {from: accounts[0]});
    await wallet.createTransfer(80, accounts[6], {from: accounts[0]});
    await wallet.approveTransfer(1, {from: accounts[0]});
    let senderApprovedTransactions = await wallet.getSenderApprovedTransactions({from: accounts[0]});
    assert(senderApprovedTransactions['1'][1] === true);
  });
});